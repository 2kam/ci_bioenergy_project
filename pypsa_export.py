...
def _load_bus_metadata() -> pd.DataFrame:
    ...
    path = os.path.join("results", "buses.csv")
    try:
        return pd.read_csv(path)
    except FileNotFoundError as exc:
        raise FileNotFoundError(
            f"Bus metadata not found: {path}. Run spatial_config.generate_buses_csv first."
        ) from exc
...
def _load_adoption(scenario: str) -> pd.DataFrame:
    ...
    try:
        ...
        raise FileNotFoundError(
            f"Adoption data for scenario '{scenario}' not found: {path}"
        ) from exc
